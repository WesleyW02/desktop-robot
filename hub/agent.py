...
from tools import DANGER, TOOL_FUNCS, TOOLS
...
def execute_tool(name: str, args: dict, mm=None) -> str:
    """执行单个工具（带安全确认），返回结果字符串。"""
...
def run_agent(mm: MiniMaxClient, messages: list, max_iter: int = MAX_TOOL_ITER) -> str:
    ...
    for _ in range(max_iter):
        ...
        for tc in tool_calls:
            ...
            try:
                ...
                args = {}
            result = execute_tool(name, args, mm=mm)
...
def main() -> int:
    ...
    print("=" * 50)
    print("  小萌 Agent 模式（Phase 3 · 可控制电脑）")
